...
public class ImportTool extends com.cloudera.sqoop.tool.BaseSqoopTool {
	...
	/**
	 * Return the max value in the incremental-import test column. This value
	 * must be numeric.
	 */
	private Object getMaxColumnId(SqoopOptions options) throws SQLException {
		...
		sb.append(options.getPrefixDecorated());
		...
		if (options.getTableName() != null) {
			...
			sb.append(") ");
			sb.append(" " + options.getUseAlias() + " ");
			sb.append(options.getSuffixDecorated());
			if (options.getUseAlias() == null
					|| options.getUseAlias().equals("")) {
		...
		ResultSet rs = null;
		try {
			LOG.info("Maximal id query for free form incremental import: "
					+ query);
	...
	private boolean isDateTimeColumn(int columnType) {
		return (columnType == Types.TIMESTAMP) || (columnType == Types.DATE)
	...
	private boolean initIncrementalConstraints(SqoopOptions options,
		...
		case AppendRows:
			try {
				...
				if (isDateTimeColumn(checkColumnType)) {
					nextIncrementalValue = (nextVal == null) ? null : manager
							.datetimeToQueryString(nextVal.toString(),
									checkColumnType);
					...
					nextIncrementalValue = (nextVal == null) ? null : nextVal
							.toString();
				}
			} catch (SQLException sqlE) {
		...
		case DateLastModified:
			...
			nextIncrementalValue = manager.datetimeToQueryString(
					nextVal.toString(), checkColumnType);
			break;
		default:
		...
		if (isDateTimeColumn(checkColumnType) && null != prevEndpoint
				&& !prevEndpoint.startsWith("\'")
				&& !prevEndpoint.endsWith("\'")) {
		...
		String checkColName = manager.escapeColName(options
				.getIncrementalTestColumn());
		...
		if (options.getTableName() != null) {
			...
			String newQuery = options.getSqlQuery().replace("$CONDITIONS",
					sb.toString());
		...
		recordOptions.setIncrementalLastValue((nextVal == null) ? null
				: nextVal.toString());
	...
	protected boolean importTable(SqoopOptions options, String tableName,
		...
		if (options.doHiveImport()) {
			hiveImport
					.importTable(tableName, options.getHiveTableName(), false);
		}
	...
	/**
	 * @return the output path for the imported files; in append mode this will
	 *         point to a temporary folder. if importing to hbase, this may
	 *         return null.
	 */
	...
	public int run(SqoopOptions options) {
		...
		try {
			if (options.doHiveImport()) {
				hiveImport = new HiveImport(options, manager,
	...
	/**
	 * Construct the set of options that control imports, either of one table or
	 * a batch of tables.
	 * 
	...
	protected RelatedOptions getImportOptions() {
		// Imports
		RelatedOptions importOpts = new RelatedOptions(
				"Import control arguments");

		importOpts.addOption(OptionBuilder
				.withDescription("Use direct import fast path")
				.withLongOpt(DIRECT_ARG).create());
		...
		if (!allTables) {
			...
			importOpts.addOption(OptionBuilder.withArgName("col,col,col...")
					...
					.withLongOpt(COLUMNS_ARG).create());
			importOpts.addOption(OptionBuilder
					.withArgName("column-name")
					.hasArg()
					.withDescription(
							"Column of the table used to split work units")
					.withLongOpt(SPLIT_BY_ARG).create());
			importOpts.addOption(OptionBuilder.withArgName("where clause")
					.hasArg()
					.withDescription("WHERE clause to use during import")
					.withLongOpt(WHERE_ARG).create());
			importOpts.addOption(OptionBuilder
					.withDescription("Imports data in append mode")
					.withLongOpt(APPEND_ARG).create());
			importOpts.addOption(OptionBuilder
					.withDescription("Imports data in delete mode")
					.withLongOpt(DELETE_ARG).create());
			importOpts.addOption(OptionBuilder.withArgName("dir").hasArg()
					.withDescription("HDFS plain table destination")
					.withLongOpt(TARGET_DIR_ARG).create());
			...
			importOpts.addOption(OptionBuilder
					...
					.hasArg()
					.withDescription(
							"Prefix to add on head of each sql statement")
					.withLongOpt(PREFIX_SQL__ARG).create());
			importOpts.addOption(OptionBuilder.withArgName("suffix").hasArg()
					.withDescription("String to append on each sql statement")
					.withLongOpt(SUFFIX_SQL__ARG).create());
			importOpts
					.addOption(OptionBuilder
							.withArgName("alias")
							.hasArg()
							.withDescription(
									"Use alias for nested query (to be compatible with sql standard)")
							.withLongOpt(USE_ALIAS_SQL__ARG).create());
			importOpts.addOption(OptionBuilder
					.withArgName("statement")
					.hasArg()
					.withDescription(
							"Set boundary query for retrieving max and min"
		...
		importOpts.addOption(OptionBuilder.withArgName("dir").hasArg()
				.withDescription("HDFS parent for table destination")
				.withLongOpt(WAREHOUSE_DIR_ARG).create());
		importOpts.addOption(OptionBuilder
		...
		importOpts.addOption(OptionBuilder.withArgName("n").hasArg()
				.withDescription("Use 'n' map tasks to import in parallel")
				.withLongOpt(NUM_MAPPERS_ARG).create(NUM_MAPPERS_SHORT_ARG));
		importOpts.addOption(OptionBuilder.withArgName("name").hasArg()
				.withDescription("Set name for generated mapreduce job")
				.withLongOpt(MAPREDUCE_JOB_NAME).create());
		importOpts.addOption(OptionBuilder
				.withDescription("Enable compression")
				.withLongOpt(COMPRESS_ARG).create(COMPRESS_SHORT_ARG));
		...
		importOpts.addOption(OptionBuilder
				...
				.hasArg()
				.withDescription(
						"Set number 'n' of rows to fetch from the "
	...
	protected RelatedOptions getIncrementalOptions() {
		RelatedOptions incrementalOpts = new RelatedOptions(
				"Incremental import arguments");

		incrementalOpts
				.addOption(OptionBuilder
						.withArgName("import-type")
						.hasArg()
						.withDescription(
								"Define an incremental import of type 'append' or 'lastmodified'")
						.withLongOpt(INCREMENT_TYPE_ARG).create());
		incrementalOpts.addOption(OptionBuilder
				.withArgName("column")
				.hasArg()
				.withDescription(
						"Source column to check for incremental change")
				.withLongOpt(INCREMENT_COL_ARG).create());
		incrementalOpts.addOption(OptionBuilder
				.withArgName("value")
				.hasArg()
				.withDescription(
						"Last imported value in the incremental check column")
	...
	public void configureOptions(ToolOptions toolOptions) {
		...
		codeGenOpts.addOption(OptionBuilder.withArgName("file").hasArg()
				...
				.withLongOpt(JAR_FILE_NAME_ARG).create());
	...
	public void printHelp(ToolOptions toolOptions) {
		...
		if (allTables) {
			...
			System.out
					.println("At minimum, you must specify --connect and --table");
		}

		System.out
				.println("Arguments to mysqldump and other subprograms may be supplied");
	...
	private void applyIncrementalOptions(CommandLine in, SqoopOptions out)
		...
		if (in.hasOption(INCREMENT_TYPE_ARG)) {
			...
			if ("append".equals(incrementalTypeStr)) {
				...
				// This argument implies ability to append to the same
				// directory.
				...
				throw new InvalidOptionsException(
						"Unknown incremental import mode: "
								+ incrementalTypeStr
								+ ". Use 'append' or 'lastmodified'."
								+ HELP_STR);
			}
		}
		...
		if (in.hasOption(INCREMENT_LAST_VAL_ARG)) {
			out.setIncrementalLastValue(in
					.getOptionValue(INCREMENT_LAST_VAL_ARG));
		}
	}
	...
	public void applyOptions(CommandLine in, SqoopOptions out)
		...
		try {
			...
			if (!allTables) {
				...
				if (in.hasOption(PREFIX_SQL__ARG)) {
					out.setPrefix(in.getOptionValue(PREFIX_SQL__ARG));
				}

				if (in.hasOption(SUFFIX_SQL__ARG)) {
					out.setSuffix(in.getOptionValue(SUFFIX_SQL__ARG));
				}

				if (in.hasOption(USE_ALIAS_SQL__ARG)) {
					out.setUseAlias(in.getOptionValue(USE_ALIAS_SQL__ARG));
				}
			...
			if (in.hasOption(NUM_MAPPERS_ARG)) {
				out.setNumMappers(Integer.parseInt(in
						.getOptionValue(NUM_MAPPERS_ARG)));
			}
			...
			if (in.hasOption(COMPRESSION_CODEC_ARG)) {
				out.setCompressionCodec(in
						.getOptionValue(COMPRESSION_CODEC_ARG));
			}

			if (in.hasOption(DIRECT_SPLIT_SIZE_ARG)) {
				out.setDirectSplitSize(Long.parseLong(in
						.getOptionValue(DIRECT_SPLIT_SIZE_ARG)));
			}

			if (in.hasOption(INLINE_LOB_LIMIT_ARG)) {
				out.setInlineLobLimit(Long.parseLong(in
						.getOptionValue(INLINE_LOB_LIMIT_ARG)));
			}
	...
	protected void validateImportOptions(SqoopOptions options)
		...
		if (!allTables && options.getTableName() == null
			...
			throw new InvalidOptionsException("--table or --" + SQL_QUERY_ARG
			...
			throw new InvalidOptionsException(
					"Jar specified with --jar-file, but no "
			...
			throw new InvalidOptionsException(
				...
				&& options.getSqlQuery() != null) {
			throw new InvalidOptionsException("Cannot specify --"
					+ SQL_QUERY_ARG + " and --table together." + HELP_STR);
		} else if (options.getSqlQuery() != null
				&& options.getTargetDir() == null
				&& options.getHBaseTable() == null) {
			throw new InvalidOptionsException(
					"Must specify destination with --target-dir." + HELP_STR);
		...
		} else if (options.getIncrementalMode() != SqoopOptions.IncrementalMode.None
				&& options.isValidationEnabled()) {
			throw new InvalidOptionsException(
					"Validation is not supported for "
			...
			throw new InvalidOptionsException(
					"--hcatalog-table cannot be used "
							+ " --warehouse-dir or --target-dir options");
		} else if (options.isDeleteMode() && options.isAppendMode()) {
			throw new InvalidOptionsException(
					"--append and --delete-target-dir can"
							+ " not be used together.");
		} else if (options.isDeleteMode()
				&& options.getIncrementalMode() != SqoopOptions.IncrementalMode.None) {
			throw new InvalidOptionsException(
					"--delete-target-dir can not be used"
							+ " with incremental imports.");
		} else if (options.getSqlQuery() == null && options.getPrefix() != null) {
			throw new InvalidOptionsException("--prefix can not be used"
					+ " without " + SQL_QUERY_ARG + " parameters.");
		} else if (options.getSqlQuery() == null && options.getSuffix() != null) {
			throw new InvalidOptionsException("--suffix can not be used"
					+ " without " + SQL_QUERY_ARG + " parameters.");
